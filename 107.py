...
matrix = [[-1,-1,-1,427,668,495,377,678,-1,177,-1,-1,870,-1,869,624,300,609,131,-1,251,-1,-1,-1,856,221,514,-1,591,762,182,56,-1,884,412,273,636,-1,-1,774], 
...
for i in range(1, len(matrix)):
    ...
    trianglemartix.append(buf)


g = Graph(len(matrix))

for x in range(len(matrix)):
    g.add_vertex_data(x, "N" + str(x))

oricost = 0
for i in range(len(trianglemartix)):
    for j in range(len(trianglemartix[i])):
        if trianglemartix[i][j] != -1 :
            g.add_edge(i + 1, j, trianglemartix[i][j])
            oricost += trianglemartix[i][j]

tree = g.prims_algorithm()
mst = g.adj_matrix
treevalue = sum(tree[1])

print(tree)
saved = oricost - treevalue
print("Result", saved)
